                    ...
                    .addCallback(object : RoomDatabase.Callback() {
                        override fun onOpen(db: SupportSQLiteDatabase) {
                            ...
                            try {
                                ...
                                // If user_profile is still empty, seed default profile so user is never blocked
                                if (profileCount == 0) {
                                    db.execSQL(
                                        "INSERT OR IGNORE INTO user_profile (id, clinicianName, role, isDarkMode, createdAt) VALUES (1, 'Dr. Rudra Goswami', 'Doctor', 1, ?)",
                                        arrayOf(System.currentTimeMillis())
                                    )
                                    Log.d(TAG, "Seeded default profile Dr. Rudra Goswami in v2 database")
                                } else {
                                    // Update legacy placeholder name to Dr. Rudra Goswami
                                    db.execSQL(
                                        "UPDATE user_profile SET clinicianName = 'Dr. Rudra Goswami' WHERE clinicianName = 'Dr. Clinician' OR clinicianName = 'Dr. Test' OR clinicianName = ''"
                                    )
                                }
                            } catch (e: Exception) {
